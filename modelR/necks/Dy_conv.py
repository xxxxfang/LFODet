...
#     def __init__(self, in_planes, out_planes, kernel_size=3, ratio=0.25, stride=1, padding=1, dilation=1, groups=1, bias=False, K=1,temperature=34, init_weight=True):
...
#         return output
# class Dynamic_conv2d(nn.Module):
#     def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=False , K=4, ):
#         super(Dynamic_conv2d, self).__init__()
#         assert in_planes % groups == 0
#         self.in_planes = in_planes
#         self.out_planes = out_planes
...
#
#         # 动态卷积的权重的生成， 生成的是batch_size个卷积参数（每个参数不同）
#         aggregate_weight = torch.mm(softmax_attention, weight).view(self.out_planes*batch_size, -1, self.kernel_size,
#                                                                     self.kernel_size)
#         if self.bias is not None:
#             aggregate_bias = torch.mm(softmax_attention, self.bias).view(-1)
#             output = F.conv2d(x, weight=aggregate_weight, bias=aggregate_bias, stride=self.stride, padding=self.padding,
...
class GhostModule_SE(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True, mode=None, args=None):
        ...
        self.conv = Dynamic_conv2d(inp, oup)
...
class GSE(nn.Module):
    def __init__(self, in_chs, mid_chs, out_chs, dw_kernel_size=3,
        ...
        self.conv = Dynamic_conv2d(mid_chs, out_chs)
...
class FC2_CSA_DRF_FPN(nn.Module):
    def __init__(self, fileters_in, model_size=1):
        ...
        #self.__conv0up2_head = nn.Conv2d(fm_0, fm_2, kernel_size=1, stride=1, padding=0)
        self.__conv0up2_head =Dynamic_conv2d(fi_0+fi_1, fm_2)
        #self.__conv0up2_head =GhostModule_original(fi_0+fi_1,fm_2,relu=True, args=None)
        self.__upsample0_2_head_1 = Upsample(scale_factor=2)
        self.__upsample0_2_head_2 = Upsample(scale_factor=2)
        self.__conv0up1_head = Dynamic_conv2d(fi_0 + fi_1, fm_1)
        # medium
        ...
        self.__conv1 = Convolutional(filters_in=2*fi_1, filters_out=fm_1, kernel_size=1, stride=1, pad=0, norm="bn",
                      activate="leaky")
        self.__conv1up2_head = Dynamic_conv2d(fi_1*2, fm_2)
        self.__conv1down0_head = Dynamic_conv2d(fi_1*2, fm_0)
        ...
        self.__conv2down0_head = Dynamic_conv2d(fi_2+fi_1, fm_0)
        #self.__conv2down0_head = GhostModule_original(fi_2+fi_1, fm_0, relu=True, args=None)
        self.__downsample2_0_head_1 = Downsample()
        self.__downsample2_0_head_2 = Downsample()
        self.__conv2down1_head = Dynamic_conv2d(fi_2 + fi_1, fm_1)
        #权重
