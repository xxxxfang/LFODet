...
for label in label_list:
    ...
    with open(label_info_path) as label_info:
        ...
        each_boxes = (r"D:\FangX24\code\LO-Det-main\mnt\Datasets\DOTA/JPEGImages/" + label.split('.')[0] + '.'
                      + label.split('.')[1] + '.png' + " " + " ".join(new_boxes) + "\n")
        temp = r"D:\FangX24\code\LO-Det-main\mnt\Datasets/DOTA/JPEGImages/" + label.split('.')[0] + '.' + label.split('.')[1] + '.png' + " " + "\n"
        if each_boxes != temp:
            boxes.append(each_boxes)

    with open(r"D:\FangX24\code\LO-Det-main\mnt\Datasets\DOTA\ImageSets/train.txt", 'a') as file:
