...
from modelR.Three_Head_lodet_hbb import LODet
...
class Trainer(object):
    def __init__(self,  weight_path, resume, gpu_id):
        ...
        self.train_dataset = data.Construct_Dataset(anno_file_type="train_s1", img_size=cfg.TRAIN["TRAIN_IMG_SIZE"])
        # self.train_dataset = data.Construct_Dataset(anno_file_type="test1500", img_size=cfg.TRAIN["TRAIN_IMG_SIZE"])
        ...
        net_model = LODet(pre_weights=self.weight_path)
    ...
    def __load_model_weights(self, weight_path,resume):
        ...
        #     print("Keys in checkpoint:", chkpt.keys())  # 打印所有的键
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox,
                ...
                imgs = imgs.to(self.device)#10,3,800,800
                ...
                # loss, loss_iou, loss_conf, loss_cls = self.criterion(p, p_d, label_lbbox,lbboxes)
            ...
            if epoch >= 120 and epoch % 10 == 0 and cfg.TRAIN["EVAL_TYPE"] == 'VOC':
                ...
                with torch.no_grad():
                    ...
                    for i in APs:
                        ...
                        mAP += APs[i]
                    mAP = mAP / (self.train_dataset.num_classes-3)
                    logger.info("mAP : {}".format(mAP))
                    logger.info("inference time: {:.2f} ms".format(inference_time))
                    writer.add_scalar('mAP', mAP, epoch)

            # elif epoch >= 60 and epoch % 10 == 0 and cfg.TRAIN["EVAL_TYPE"] == 'COCO':
            #     logger.info("===== Validate =====".format(epoch, self.epochs))
            #     with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_path', type=str, default='weight/ShuffleNetV2.1.5x.pth',
                        help='weight file path') #weight/  mobilenetv2_1.0-0c6065bc    mobilenetv3-large-1cd25616.pth  ghost_93.98
    parser.add_argument('--resume', action='store_true',default=False,  help='resume training flag')
    # parser.add_argument('--weight_path', type=str, default='weight/last1.pt',
    ...
    parser.add_argument('--log_path', type=str, default='04log/log_fea', help='log path')
    opt = parser.parse_args()
    writer = SummaryWriter(logdir=opt.log_path + '/event')
    logger = Logger(log_file_name=opt.log_path + '/n_04_shu.txt', log_level=logging.DEBUG, logger_name='LODet-hbb').get_log()
