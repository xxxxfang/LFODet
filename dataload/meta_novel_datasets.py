...
class Meta_Construct_Dataset(Dataset):
    def __init__(self, anno_file_type_s,anno_file_type, img_size=448):
        ...
        #novel_path = os.path.join(self.data_dir, 'novel_cat' , '{}'.format(self.anno_file_type) )
        novel_path = os.path.join(self.data_dir,anno_file_type_s,'{}'.format(self.anno_file_type))
        ...
        for path in novel_files:
            ...
            with open(path, 'r') as f:
                ...
                for img_id in img_ids:
                    if len(task['samples']) < cfg.Kshot *2:
        ...
        #self.__annotations_s = self.tasks
    def __len__(self):
        #return len(self.__annotations)
        return self.num_tasks

    #添加index为task中的顺序编号（很多图片），item为图像anno的具体那个图像
    def __getitem__(self, item):#对一个task内的所有图片进行融合
        img_ids = self.tasks[item]['samples']#图像的id？直接转换为图片的名称+锚框
        #img_cls = self.tasks[item]['cls_id']#图像属于的类别
        #file_names = img_ids
        #ann_ids = img_ids
        imgs = []#rets = []
        label_sbbox_list = []
        ...
        for i in range(len(img_ids)):
            #img_id = img_ids[i]
            img_org, bboxes_org = self.__parse_annotation(img_ids[i])
            img_org = img_org.transpose(2, 0, 1)  # HWC->CHW

            item_mix = random.randint(0, len(img_ids) - 1)
            img_mix, bboxes_mix = self.__parse_annotation(img_ids[item_mix])
            ...
            # 存储结果和def concatRet(self, rets):功能一样
            imgs.append(img)
            label_sbbox_list.append(label_sbbox)
            label_mbbox_list.append(label_mbbox)
            label_lbbox_list.append(label_lbbox)
            sbboxes_list.append(sbboxes)
            mbboxes_list.append(mbboxes)
            lbboxes_list.append(lbboxes)

        imgs_tensor = torch.from_numpy(np.stack(imgs))
        label_sbbox_tensor = torch.from_numpy(np.stack(label_sbbox_list))
        label_mbbox_tensor = torch.from_numpy(np.stack(label_mbbox_list))
        label_lbbox_tensor = torch.from_numpy(np.stack(label_lbbox_list))
        sbboxes_tensor = torch.from_numpy(np.stack(sbboxes_list))
        mbboxes_tensor = torch.from_numpy(np.stack(mbboxes_list))
        lbboxes_tensor = torch.from_numpy(np.stack(lbboxes_list))

        return imgs_tensor, label_sbbox_tensor, label_mbbox_tensor, label_lbbox_tensor, sbboxes_tensor, mbboxes_tensor, lbboxes_tensor
    # def __load_annotations(self, anno_type):
    #     assert anno_type in ['train', 'val', 'test']
    #     anno_path = os.path.join(cfg.PROJECT_PATH, 'ImageSets', anno_type + ".txt")
...
if __name__ == '__main__':
    ...
    train_dataset=Meta_Construct_Dataset(anno_file_type="train", img_size=cfg.TRAIN["TRAIN_IMG_SIZE"])
    train_dataloader = DataLoader(train_dataset,batch_size=cfg.TRAIN["META_BATCH_SIZE"], num_workers=cfg.TRAIN["NUMBER_WORKERS"],shuffle=False)
