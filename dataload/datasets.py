...
class Construct_Dataset(Dataset):
    def __init__(self, anno_file_type, img_size=448):
        ...
        self.__annotations = self.__load_annotations(anno_file_type)#samples的内容

    def __len__(self):#
        return len(self.__annotations)

    def __getitem__(self, item):#混合图片
########img_id= self.__annotations[item]
        img_org, bboxes_org = self.__parse_annotation(self.__annotations[item])#img_id
    ...
    def __load_annotations(self, anno_type):#滤除空格，和空行
        assert anno_type in ['train_s2', 'train_s3','train_s4','val', 'train_s1']
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(train_dataset,batch_size=cfg.TRAIN["BATCH_SIZE"], num_workers=cfg.TRAIN["NUMBER_WORKERS"],shuffle=False)
