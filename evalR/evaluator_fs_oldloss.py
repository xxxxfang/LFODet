...
class Evaluator(object):
    ...
    def APs_voc(self, multi_test=False, flip_test=False):  # 相当于一个解码的过程并将结果保存
        ...
        APs, Recalls = self.__calc_APs(iou_thresh=self.iouthresh_test)
        return APs, Recalls, self.inference_time
        #return self.__calc_APs(iou_thresh=self.iouthresh_test)
    ...
    def __predict(self, img, test_shape, valid_scale):  # 得到转换后的边界框#包含在解码的过程中
        ...
        with torch.no_grad():
            ...
            if self.showheatmap:
                _, p_d, beta = self.model(img)  # 显示热图？可视化
            else:
