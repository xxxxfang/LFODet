...
from evalR.evaluator_fs import *
...
class Trainer(object):
    def __init__(self,  weight_path, resume,cmp, gpu_id):
        init_seeds(0)
        self.prune=0
        self.sr=True
        ...
        self.train_dataset = data.Fs_Construct_Dataset(anno_file_type_s="random5",anno_file_type="fs_3", img_size=cfg.TRAIN["TRAIN_IMG_SIZE"])
        ...
        if cmp:
            self.__load_model_weights_cmp(weight_path, resume)
        else:
            self.__load_model_weights(weight_path, resume)
            if not resume:
                self.__load_model_weights_base(cfg.weight_path, resume)
        self.model.train()
    ...
    def __load_model_weights(self, weight_path, resume):
        if resume:
            ...
            # key_mapping = {
            #     'learner_s.conv_learn.weight': ['conv_head_s_b.weight','conv_head_s.weight'],
            #     'learner_l.conv_learn.weight': ['conv_head_l_b.weight','conv_head_l.weight'],
            #     'learner_s.conv_learn.bias': ['conv_head_s_b.bias','conv_head_s.bias'],
            #     'learner_l.conv_learn.bias': ['conv_head_l_b.bias','conv_head_l.bias'],
            # }
            key_mapping = {
            ...
            del chkpt
    def __load_model_weights_base(self, weight_path, resume):
        # if resume:
        #     last_weight = weight_path
        #     chkpt = torch.load(last_weight, map_location=self.device)
        #     self.model.load_state_dict(chkpt['model'])
        ...
        # else:
            ...
            state_dict_ = chkpt['model']
            key_mapping = {
                'conv_head_s.weight': 'conv_head_s_b.weight',
                'conv_head_l.weight': 'conv_head_l_b.weight',
                'conv_head_m.weight': 'conv_head_m_b.weight',
                'conv_head_s.bias': 'conv_head_s_b.bias',
                'conv_head_l.bias': 'conv_head_l_b.bias',
                'conv_head_m.bias': 'conv_head_m_b.bias',
            }
            # 替换权重字典中的键名
            updated_chkpt = {}
            for key, value in state_dict_.items():
            ...
            print("loading weight file is done")
            del chkpt
    def __load_model_weights_cmp(self, weight_path, resume):
        if resume:
            last_weight = weight_path
            chkpt = torch.load(last_weight, map_location=self.device)
            self.model.load_state_dict(chkpt['model'])
            ...
            state_dict_ = chkpt['model']
            key_mapping = {
                'conv_head_s.weight': ['conv_head_s_b.weight','conv_head_s.weight'],
                'conv_head_l.weight': ['conv_head_l_b.weight','conv_head_l.weight'],
                'conv_head_s.bias': ['conv_head_s_b.bias','conv_head_s.bias'],
                'conv_head_l.bias': ['conv_head_l_b.bias','conv_head_l.bias'],
            }
            # 替换权重字典中的键名
            updated_chkpt = {}
            for key, value in state_dict_.items():
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if mode==11:
                if Kshot == 10:
                    self.warm_up_epochs = 35#15（2head）15_有效20
                elif Kshot == 5:
                    self.warm_up_epochs = 25#10_20，11——25
                elif Kshot == 3:
                    self.warm_up_epochs = 50#50
                # elif cfg.Kshot == 20:
                ...
                if warm_up:
                    if Kshot == 10:
                        ...
                        if (ep - 1) < self.warm_up_epochs:
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] = cfg.lr_fs * (ep / self.warm_up_epochs)
                        if ep - 1 == 40:#30_11  10——25
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] *= 0.2
                        if ep - 1 == 70:#50_11  10——40
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] *= 0.2
                    elif Kshot ==3:#25
                        if (ep - 1) < self.warm_up_epochs:
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] = cfg.lr_fs * (ep / self.warm_up_epochs)
                        if ep - 1 == 50:#30_11  10——25  60
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] *= 0.2
                        if ep - 1 == 60:#50_11  10——40  120
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] *= 0.2
                # elif cfg.Kshot == 20:#5
                #     if (ep - 1) < self.warm_up_epochs:#5
                #         for param_group in self.optimizer.param_groups:
                #             param_group['lr'] = cfg.lr_fs * (ep / self.warm_up_epochs)
                #     if ep - 1 == 10:  # 假设设定在某个适当的地方降低学习率
                #         for param_group in self.optimizer.param_groups:
                #             param_group['lr'] *= 0.2
                #     if ep - 1 == 30:  # 同样增加这个条件
                #         for param_group in self.optimizer.param_groups:
                ...
                if Kshot == 10:
                    self.warm_up_epochs = 15#15（2head）15_有效20
                elif Kshot == 5:
                    self.warm_up_epochs = 20#10_20，11——25
                # elif cfg.Kshot == 20:
                #     self.warm_up_epochs = 5
                elif Kshot == 3:
                ...
                if warm_up:
                    if Kshot == 10:
                        ...
                        if (ep - 1) < self.warm_up_epochs:
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] = cfg.lr_fs * (ep / self.warm_up_epochs)
                        if ep - 1 == 25:#30_11  10——25
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] *= 0.2
                        if ep - 1 == 40:#50_11  10——40
                            for param_group in self.optimizer.param_groups:
                                param_group['lr'] *= 0.2
                    elif Kshot ==3:#25
                        if (ep - 1) < self.warm_up_epochs:
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox,
                ...
                p, p_d = self.model(imgs,switch = 'tunning')
                # p_bs, p_d_bs =self.model(imgs,switch = 'base')
            ...
            #     novel_list = ['airplane','baseballfield','tenniscourt','trainstation','windmill']
            #     with torch.no_grad():
            #         APs,nms_time = Evaluator(self.model).APs_voc(self.__multi_scale_test, self.__flip_test)
            #
            #         for i in APs:
            ...
            if epoch >= 300 and epoch % 20 == 0:
                ...
                with torch.no_grad():
                    ...
                    # mAP = mAP / (self.train_dataset.num_classes - 7)
                    # AP = AP / (self.train_dataset.num_classes)
                    # bAP = bAP / (self.train_dataset.num_classes - 3)
                    mAP = mAP / (self.train_dataset.num_classes - 15)
                    AP = AP / (self.train_dataset.num_classes)
                    bAP = bAP / (self.train_dataset.num_classes - 5)
                    logger.info('nAP:%g' % (mAP))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cmp', action='store_true',default=False,  help='resume training flag')
    parser.add_argument('--gpu_id', type=int, default=0, help='gpu id')
    parser.add_argument('--log_path', type=str, default='04log/fs/ran5', help='log path')
    parser.add_argument('--arch', type=str, default='resdcn_101', help='arch path')
    opt = parser.parse_args()
    Kshot=3
    mode=11
    writer = SummaryWriter(logdir=opt.log_path + '/event')
    logger = Logger(log_file_name=opt.log_path + '/DIOR_04_K3_11_changelr_lr.txt', log_level=logging.DEBUG, logger_name='mob').get_log()
                                                #更新！
    Trainer(weight_path=opt.weight_path, resume=opt.resume, cmp=opt.cmp,  gpu_id=opt.gpu_id).train()
